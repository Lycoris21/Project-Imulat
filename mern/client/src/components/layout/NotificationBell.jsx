...
export default function NotificationBell({ user, notifications, setNotifications, refreshNotifications }) {
  ...
  const handleBellClick = () => {
    if (!open) {
      setOpen(true); // This triggers the open effect
    } else {
      handleClose();
    }
  };
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div
          className={`absolute right-0 mt-2 w-80 bg-white shadow-lg border rounded-lg z-50 origin-top-right transform transition-all duration-150 ${
