...
export default function NotificationItem({ notification, onClose, refreshNotifications }) {
  ...
  const getIcon = (type) => {
    const iconMap = {
      ...
      peer_review: "text-purple-500",
      report_approved: "text-emerald-600",
    ...
    return (
      <svg className={`w-4 h-4 ${color}`} fill="currentColor" viewBox="0 0 20 20">
        <path
          fillRule="evenodd"
          d={
            type === 'report' || type === 'report_approved' || type === 'report_rejected'
  ...
  const getMessage = () => {
    const username = notification.senderId?.username || "Unknown";
