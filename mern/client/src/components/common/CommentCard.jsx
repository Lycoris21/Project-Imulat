...
export default function CommentCard({ comment, variant = "default" }) {
  ...
  return (
    <div className={`bg-white rounded-lg shadow-sm border hover:shadow-md transition-shadow ${isCompact ? 'p-4' : 'p-6'}`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          {comment.targetId && (
            <div className="mb-3">
              <p className="text-sm text-gray-600">
                ...
                <Link
                  to={`/${comment.targetType}s/${comment.targetId._id}${comment._id ? `?highlight=${comment._id}` : ''}`}
          ...
          <div className="flex items-center space-x-4 text-sm text-gray-500">
            <div className="flex items-center space-x-1">
              ...
              <span>{comment.reactionCounts?.like || comment.likes || 0}</span>
            </div>
            <div className="flex items-center space-x-1">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <span>{comment.reactionCounts?.dislike || comment.dislikes || 0}</span>
            </div>
            <div className="flex items-center space-x-1">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
