...
const ClaimCardCompact = ({ claim }) => (
  <Link to={`/claims/${claim.id}`} className="block border-b border-gray-200 pb-4 last:border-b-0 shadow-sm hover:bg-[#EDEEF1] rounded-lg p-3 m-3 transition-all duration-300 transform hover:-translate-y-1 group">
    <div className="flex justify-between items-start mb-2">
      <h3 className="font-bold text-gray-800 text-sm leading-tight flex-1 mr-2 group-hover:text-selected transition-colors">
        {claim.claim}
      </h3>
