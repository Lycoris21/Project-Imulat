      ...
      const pipeline = [
        { $match: matchCriteria },
        {
          $lookup: {
            ...
            let: { reportId: '$_id' },
            pipeline: [
              { $match: { $expr: { $and: [{ $eq: ['$targetType', 'report'] }, { $eq: ['$targetId', '$$reportId'] }] } } }
            ],
            as: 'comments'
          }
        },
        {
          $lookup: {
            ...
            let: { reportId: '$_id' },
            pipeline: [
              { $match: { $expr: { $and: [{ $eq: ['$targetType', 'report'] }, { $eq: ['$targetId', '$$reportId'] }, { $eq: ['$reactionType', 'like'] }] } } }
            ],
            as: 'likes'
          }
        },
        ...
        },
        {
          $lookup: {
            ...
            foreignField: '_id',
            as: 'claimIds',
            pipeline: [{ $project: { claimTitle: 1, aiTruthIndex: 1 } }]
          }
        },
        {
          $addFields: {
